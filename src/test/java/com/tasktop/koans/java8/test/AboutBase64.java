...
public class AboutBase64 {
	...
	public void encodeToBase64() {
		...
		String myString = new String(encoded); // FIXME: create the string using the encoded bytes -- FIXED
		assertEquals("bXktc3RyaW5n", myString);
	}

	@Test
	public void decodeFromBase64() {
		// FIXME: Where do we get an encoded string that means "my-string"? One hint... look up! -- FIXED
		byte[] decoded = Base64.getDecoder().decode(Base64.getEncoder().encode("my-string".getBytes()));
