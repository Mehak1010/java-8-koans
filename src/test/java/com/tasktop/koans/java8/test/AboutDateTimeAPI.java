...
public class AboutDateTimeAPI {
	...
	public void java8_datesAreImmutable() {
		...
		LocalDateTime date2 = date.plus(durationtoAdd); // FIXME: See if you can find a way to add the durationToAdd to the date -- FIXED
	...
	public void java8_timeInMiliSeconds() {
		...
		long timeInMilis = now.toEpochMilli(); // FIXME: checkout if you can find the method on "now" that returns the milis -- FIXED
	...
	public void java8_getNow() {
		LocalDate now = LocalDate.now(); // FIXME: maybe there is a factory method on LocalDate you can use to get now -- FIXED
	...
	public void java8_dateInFuture() {
		...
		LocalDate future = now.plus(1, ChronoUnit.DAYS); // FIXME: Find out what a ChronoUnit is and what it means. -- FIXED
	...
	public void java8_createSpecificDate() {
		LocalDate now = LocalDate.of(2000, 11, 23); // FIXME: use the of method to create a date that represents 2000-11-23 -- FIXED
	...
	public void java8_convertDateToDateTime() {
		...
		// FIXME: A LocalDate does not store hours and minutes. See if you can enhance now with this information. -- FIXED
		// One hint, take a look at the atTime method.
		LocalDateTime time = LocalDateTime.of(now.getYear(), now.getMonthValue(), now.getDayOfMonth(), 14, 14);
	...
	public void java8_getNowWithSpecificClock() {
		Clock clock = Clock.fixed(Instant.EPOCH, ZoneId.systemDefault()); // FIXME: create a clock that start at the epoch and uses the sytem timezone -- FIXED
	...
	public void java8_periodsAreUsedToDescribeDurationsInFullDays() {
		...
		Period period = Period.between(now, future); // FIXME: calculate the period between now and future -- FIXED
	...
	public void java8_durationsAreUsedToDescribeDurationsMorePrecisely() {
		...
		Duration duration = Duration.between(now, future); // FIXME: calculate the duration between now and future -- FIXED
	...
	public void java8_durationsCanAlsoBeUsedToCreateDate() {
		...
		LocalDateTime future = now.plus(duration); // FIXME: add the duration to now -- FIXED
	...
	public void java8_adjustDatesBySemantic() {
		...
		LocalDate saturday = date.with(TemporalAdjusters.next(DayOfWeek.SATURDAY)); // FIXME: find out how to get the next Saturday. -- FIXED
	...
	public void java8_useFixedOffsetsForTimeZones() {
		// FIXME: I wonder how many hours we need to pass in to have 28800 seconds -- FIXED
	...
	public void java8_useZoneIdsForTimeZones() {
		ZonedDateTime berlin = ZonedDateTime.now(ZoneId.of("Europe/Berlin")); // FIXME: How do we find out the ZoneIf of Berlin? -- FIXED
